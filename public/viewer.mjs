	...
	connectRoot(newRoot) {
		...
		if (this.windowElement) {
          ...
          DOMLocalization already has a root from a different window.`)
			...
			this.mutationObserver = new this.windowElement.MutationObserver((mutations) =>
...
class NewAltTextManager {
	...
	#toggleTitleAndDisclaimer() {
		const visible =
			this.#isAILoading || (this.#guessedAltText && this.#guessedAltText === this.#textarea.value)
	...
	#setProgress() {
		...
		const callback = async ({ detail: { finished, total, totalLoaded } }) => {
			...
			const totalSize = (this.#downloadModelDescription.ariaValueMax = Math.round(
				total / ONE_MEGA_BYTES
			))
			const downloadedSize = (this.#downloadModelDescription.ariaValueNow = Math.round(
...
class CaretBrowsingMode {
	...
	moveCaret(isUp, select) {
		...
		const focusElement =
			focusNode.nodeType !== Node.ELEMENT_NODE ? focusNode.parentElement : focusNode
...
class PDFDocumentProperties {
	...
	async open() {
		...
		const [fileName, fileSize, title, creationDate, modificationDate, pageSize, isLinearized] =
			await Promise.all([
	...
	async #parseFileSize(b = 0) {
		...
		return kb
			? this.l10n.get(
					mb >= 1 ? 'pdfjs-document-properties-size-mb' : 'pdfjs-document-properties-size-kb',
					{
						mb,
						kb,
						b,
...
const DIACRITICS_EXCEPTION = new Set([
	0x3099, 0x309a, 0x094d, 0x09cd, 0x0a4d, 0x0acd, 0x0b4d, 0x0bcd, 0x0c4d, 0x0ccd, 0x0d3b, 0x0d3c,
	0x0d4d, 0x0dca, 0x0e3a, 0x0eba, 0x0f84, 0x1039, 0x103a, 0x1714, 0x1734, 0x17d2, 0x1a60, 0x1b44,
	0x1baa, 0x1bab, 0x1bf2, 0x1bf3, 0x2d7f, 0xa806, 0xa82c, 0xa8c4, 0xa953, 0xa9c0, 0xaaf6, 0xabed,
...
class PDFPageDetailView extends BasePDFPageView {
	...
	update({ visibleArea = null, underlyingViewUpdated = false } = {}) {
		...
		const maxDetailToVisibleLinearRatio = Math.sqrt(
			OutputScale.capPixels(maxCanvasPixels, capCanvasAreaFactor) / visiblePixels
		)
	...
	static #enableGlobalSelectionListener() {
		...
		document.addEventListener(
			...
			() => {
				...
				isFirefox ??=
					getComputedStyle(this.#textLayers.values().next().value).getPropertyValue(
						'-moz-user-select'
					) === 'none'
				if (isFirefox) {
					return
...
class PDFViewer {
	...
	scrollPageIntoView({
		...
		switch (destArray[1].name) {
			...
			case 'FitR':
				...
				heightScale =
					(this.container.clientHeight - vPadding) / height / PixelsPerInch.PDF_TO_CSS_UNITS
	...
	_getVisiblePages() {
		const views =
				this._scrollMode === ScrollMode.PAGE ? this.#scrollModePageState.pages : this._pages,
	...
	forceRendering(currentlyVisiblePages) {
		...
		const preRenderExtra =
			this._spreadMode !== SpreadMode.NONE && this._scrollMode !== ScrollMode.HORIZONTAL
...
class SignatureManager {
			...
			this.#drawSVG.addEventListener(
				...
				(evt) => {
					...
					if (drawPoints.length >= 4) {
						...
						this.#drawPathString += `C${(x1 + 5 * x2) / 6} ${(y1 + 5 * y2) / 6} ${
							(5 * x2 + x) / 6
						} ${(5 * y2 + y) / 6} ${(x2 + x) / 2} ${(y2 + y) / 2}`
					} else {
	...
	async #extractSignature(file) {
		...
		const { outline } = (this.#extractedSignatureData = this.#currentEditor.getFromImage(
			data.bitmap
		))
		if (!outline) {
			this.#dialog.classList.toggle('waiting', false)
...
class Toolbar {
	...
	#bindListeners(buttons) {
		const { eventBus } = this
		const { editorHighlightColorPicker, editorHighlightButton, pageNumber, scaleSelect } =
			this.#opts
		const self = this
	...
	async _initializeViewerComponents() {
		...
		if (AppOptions.get('enableUpdatedAddImage')) {
			altTextManager = appConfig.newAltTextDialog
				? new NewAltTextManager(appConfig.newAltTextDialog, overlayManager, eventBus)
				: null
		} else {
			altTextManager = appConfig.altTextDialog
				? new AltTextManager(appConfig.altTextDialog, container, overlayManager, eventBus)
				: null
		}
...
function onUpdateViewarea({ location }) {
	...
	if (this.appConfig.secondaryToolbar) {
		this.appConfig.secondaryToolbar.viewBookmarkButton.href = this.pdfLinkService.getAnchorUrl(
